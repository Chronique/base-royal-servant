  ...
  return (
    <div className="max-w-xl mx-auto p-4 pb-32 flex flex-col gap-6">
      <div className="bg-[#0052FF] p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden text-center">
          <p className="text-[10px] font-bold opacity-60 uppercase tracking-widest italic">Base App Shield</p>
          ...
          <div className="flex justify-center gap-2 mt-4">
             <button onClick={() => sdk.actions.addMiniApp()} className="text-[10px] bg-white/20 px-3 py-1 rounded-full font-bold transition-all">+ Pin App</button>
             {isSmartAccount && <span className="text-[10px] bg-green-400 text-blue-900 px-3 py-1 rounded-full font-black italic uppercase tracking-tighter">Gasless Ready</span>}
          </div>
      </div>
      ...
      <div className="min-h-[450px]">
        {activeTab === "scanning" && (
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
            <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest px-2">Asset Status</h3>
            {allowances.map((item) => (
              <div key={item.id} className="p-4 border-2 border-gray-100 rounded-[1.5rem] flex justify-between items-center bg-white">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-bold uppercase text-gray-400">{item.isScam ? "Suspicious" : "Verified Asset"}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        {activeTab === "score" && (
          <div className="flex flex-col gap-6 animate-in zoom-in-95">
             <div className="text-center p-10 bg-gray-50 rounded-[3rem] border-2 border-dashed border-gray-200">
                ...
                <p className="text-xs text-gray-400 font-bold uppercase mt-2">FID: {userFid || 'Not Connected'}</p>
             </div>
          </div>
        )}
      </div>
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[92%] max-w-md bg-white/90 backdrop-blur-2xl border border-gray-200 rounded-[2.5rem] p-2 shadow-2xl flex justify-around z-50">
        <button onClick={() => setActiveTab("scanning")} className={`flex-1 py-4 rounded-[2rem] flex flex-col items-center gap-1 transition-all ${activeTab === 'scanning' ? 'bg-[#0052FF] text-white shadow-xl' : 'text-gray-400'}`}>
          <Search size={20} strokeWidth={3} /><span className="text-[8px] font-black uppercase">Scan</span>
        </button>
        <button onClick={() => setActiveTab("revoke")} className={`flex-1 py-4 rounded-[2rem] flex flex-col items-center gap-1 transition-all ${activeTab === 'revoke' ? 'bg-[#0052FF] text-white shadow-xl' : 'text-gray-400'}`}>
          <Trash2 size={20} strokeWidth={3} /><span className="text-[8px] font-black uppercase">Revoke</span>
        </button>
        <button onClick={() => setActiveTab("score")} className={`flex-1 py-4 rounded-[2rem] flex flex-col items-center gap-1 transition-all ${activeTab === 'score' ? 'bg-[#0052FF] text-white shadow-xl' : 'text-gray-400'}`}>
